...
const TaskTagsPage = () => {
  const [taskTagsList, setTaskTags] = useState<string[]>([]);
  const [tags, setTags] = useState<string>("");
  const addTaskTags = () => {
    const TAGS_LIST_LIMIT = 6;
    const conditions = taskTagsList && tags.length !== 0 && tags.length! >= 6;
    if (conditions && taskTagsList.length < TAGS_LIST_LIMIT) {
      setTaskTags((prev) => [...prev, tags]);
      setTags("");
    }
  };
