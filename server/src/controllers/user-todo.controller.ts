...
// setTodoLifecycle
export const setTodoLifecycle = async (req: IExtendedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    const todoId = req.params.id;
    const { todoLifecycle } = req.body;
    ...
    const allowedLifecycles = ["active", "archived", "trashed"] as const;
    const normalizedLifecycle = String(todoLifecycle).toLowerCase();

    if (!allowedLifecycles.includes(normalizedLifecycle as any)) {
      return res.status(400).json({
        message: "Invalid todo lifecycle provided",
      });
    }
    ...
    return res.status(200).json({
      message: "Todo lifecycle updated successfully",
      data: updatedTodo,
    });
